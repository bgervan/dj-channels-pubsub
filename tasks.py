...
def coverage(c):
    ...
    """
    c.run("coverage run --source dj-channels-pubsub runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/dj-channels-pubsub.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ dj_channels_pubsub")
...
def lint(c):
    ...
    """
    c.run("flake8 dj-channels-pubsub tests")
...
def release(c, bumpsize=''):
    ...
    import dj_channels_pubsub
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=dj_channels_pubsub.__version__))
    c.run("git push --tags")
