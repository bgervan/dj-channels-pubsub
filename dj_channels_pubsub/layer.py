...
class GcePubSubChannelLayer(BaseChannelLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._subscribes = {}
        self.project = "agenda-test-a84a5"  # os.getenv('GOOGLE_CLOUD_PROJECT')
        self._messages = {}
    ...
    async def receive(self, channel):
        ...
        while len(self._messages[channel]) == 0:
            await asyncio.sleep(0.1)
        # if channel in self._messages:
        #     if self._messages[channel].qsize() > 0:
        message = self._messages[channel][0]
        message.ack()
        self._messages[channel].discard(message)
    ...
    def _receive_callback(self, channel, message):
        ...
        if channel not in self._messages:
            self._messages[channel] = OrderedSet()
        self._messages[channel].add(message)
